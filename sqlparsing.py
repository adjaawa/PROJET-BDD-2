...
'''def test () :
    ...
    sql = input ("Enter a create user statement : ")
    print ("-->", sql)

    if (sql.startswith (('Create user').casefold())) :
        tokens = parseCreateUser(sql)
        print (tokens.Username)
        print (tokens.Password)
    else :
        print ('Not a create user statement')
